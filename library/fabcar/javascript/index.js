...
app.post("/file", urlencodedparser, (req, res) => {
    async function main() {
        try {
            ...
            await parent.submitTransaction('addFile', req.body.filename, req.body.filetype, req.body.sensitivity);
...
app.post("/rule", urlencodedparser, (req, res) => {
    function getDataFromServer2(data) {
        ...
        return fetch('http://34.66.251.90:8000/rule', {
            method: 'post',
            body: jsondata,
    ...
    async function main() {
        try {
            ...
            const parent = network.getContract('library', 'Parent');
            let result = await parent.evaluateTransaction('divideIntoSubRules', 'Rule' + i, req.body.designation, req.body.department, req.body.operation, req.body.filetype, req.body.sensitivity);
            ...
            await sub.submitTransaction('addSubRule', 'Rule' + i, result[1].opera, result[1].filet, result[1].sensi);
...
app.post("/access", urlencodedparser, (req, res) => {
    function getDataFromServer2(data) {
        const jsondata = JSON.stringify(data);
        return fetch('http://34.66.251.90:8000/access', {
            method: 'post',
            body: jsondata,
            headers: { 'Content-Type': 'application/json' },
        }).then(res => res.json())
    ...
    async function main() {
        try {
            ...
            const parent = network.getContract('library', 'Parent');
            let result = await parent.evaluateTransaction('divideIntoSubRequests', req.body.rollno, req.body.operation, req.body.filename);
            result = result.toString('utf-8');
            result = JSON.parse(result);
            console.log(result);

            const sub = network.getContract('library', 'Sub');
            let result2 = await sub.evaluateTransaction('accessrequest', result[1].opera, result[1].filen);
